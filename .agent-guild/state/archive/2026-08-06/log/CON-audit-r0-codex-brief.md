# Second-opinion audit brief: CON-audit r0

You are acting as `checker-courier` relaying a constitution audit for a second opinion. Judge ONLY the material inlined below. You cannot read any repository, run any command, or see anything outside this prompt. Do not assume any command was or could be run on your side; where a clause's check is a script, judge it against the "Evidence collected locally" section.

## What to judge

The document under audit is a guild "constitution": the falsifiable standard a build job is checked against. The job it governs (GitHub issue kendrick/skills#16) will add, to an existing repo: a Tier 2 migration flow (sidecar proposals file; the model fills in `summary` and `entities`; `--tier2 <file> --apply` writes them mechanically) and a standalone `verify-migration.sh`. Neither exists yet — clauses about them constrain work about to be built, so judge those on falsifiability and check-method concreteness, not against a script on disk. Tier 1 of the migration already shipped at commit `1f17478`.

Apply this rubric to every clause C-1 through C-16:
...
2. It is falsifiable: a specific artifact that would violate it can be stated. If no failing example can be described, the clause is unfalsifiable — fail it and say why.
3. No two clauses contradict each other.
4. Protected content must point at a real manifest that parses. This constitution declares no passages manifest and says why — judge whether that reasoning holds.

Also judge coverage: does any requirement in the spec (inlined below) lack a governing clause?
...
- "findings": array of {"clause_id", "severity", "description", "evidence"} — REQUIRED non-empty if verdict is "fail"; each finding's evidence must be non-empty and cite the inlined material. May be empty on "pass". Advisory notes that do not fail a clause belong in "summary", not "findings".
- "duration_ms": null
- "cost_usd": null

---

## Document under audit: constitution.md (verbatim)
...
## The spec the constitution must cover: spec.md (verbatim)
...
## Evidence collected locally (verbatim command results from the repo)

== C-13 scope check (expect exit 0, empty output) ==
exit=0
output=[]

== C-15 key-order tripwire (expect exit 0) ==
exit=0
NOTE_KEY_ORDER words=17 (closes line 19); RECORD_KEY_ORDER words=19 (closes line 21)

== C-6 cited preflights ==
lint-scope.sh:63: if ! command -v yq >/dev/null 2>&1; then
migrate-scope.sh:76: command -v yq >/dev/null 2>&1 || {

== C-12 baseline ==
git diff 1f17478 -- tests/ : 0 lines
smoke.sh:607: require_output "$apply_out" "left alone: 0"
smoke.sh:159: require_line "$broken_out" "failures: 18" broken

== C-14 failing example still live ==
migration.md:19: **Tier 2 is generative** (summary and entities, which most v1 notes never had). It is gated separately, because a generated summary is the skill's reading of a note rather than a fact about it. Tier 2 is tracked in its own ticket and is not implemented here.
