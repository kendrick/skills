...
clause_types_sampled: mixed  # C-1,2,3,4,5,11,16 judgment; C-12,13,15 deterministic per constitution.md
...
cost:
  ...
  vendor_calls: 3
retrofit_note: >-
  vendor_calls sourced positively: vendor-calls.jsonl lines 5, 6, and 7 all
  target this same T-002-sonnet-r0-codex verdict stem — the prose below calls
  this "the third crossing," lines 5 and 6 having been killed at the Bash
  tool's 120000ms timeout before line 7 succeeded. cost fields reflect only
  the successful line 7 attempt. tokens_in/tokens_out are null because the
  courier itself omitted them on that successful run (per the prose's "the
  one field that would have settled it was left null"), not because the
  ledger lacks a line. brief_framing unknown — briefs/T-002.md and
  briefs/T-002-evidence.md are a clause/spec dump plus locally collected
  evidence, with no confirm/attack/open instruction surviving.
```
