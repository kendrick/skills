...
```yaml
clause_types_sampled: mixed  # C-14,16 judgment; C-12,13 deterministic per constitution.md
...
retrofit_note: >-
  cost fields are null in vendor-calls.jsonl line 11 itself — no
  tokens_in/tokens_out/brief_tokens despite a completed, validated crossing.
  wall_s left null too: the ledger's duration_ms (150000) carries the
  courier's own annotation "measured from timeout boundary," and the prose
  below calls this the third instance of a courier reporting timing it did
  not measure. brief_framing unknown — no brief file for this task survives
  under briefs/ or log/.
```
