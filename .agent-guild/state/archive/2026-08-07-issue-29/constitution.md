# Constitution: kendrick/skills#29 — the migration record's prose

The job changes two strings a passing `verify-migration.sh` run prints, and the
suite assertions that pin them. It is small, and the interesting risk is not the
prose: it is that the assertions pinning the prose can be updated into something
that no longer catches anything. Job #16's retrospective found every real defect
in that run in that shape — implementation right, test vacuous — and this job
edits assertions on purpose, so the clauses below weight that risk over the
wording itself.

Two open questions the spec left were settled by the user before drafting:

- The paste instruction is printed on its own line **before** the record, with a
  blank line between, so the record is the last thing the run prints (C-3).
- The summary block's `links checked: N (id fallback: M)` line is **out of
  scope**. It is a label and a value, not a sentence (C-7, non-goals).

Revised after `CON-audit-r0` returned FAIL. That audit's central finding drives
C-11: every assertion over the record today is a substring search across the
whole of `pass_out`, which cannot tell an instruction sitting on its own line
above the record from one welded back onto the record's tail. Without C-11 a
worker can satisfy every other clause and still ship a suite that would not fail
on the defect this issue was filed about.

## The passing run, and how to produce one

Three clauses below ask a checker to read a real passing run's stdout. The suite
builds one at `tests/inbox-to-memory-smoke.sh:1013-1026` and then swallows it
into `$pass_out`, printing it only on failure — so "run the passing case" needs
a route or a checker will quietly substitute reading the `printf` for observing
its output. This recipe is that route. Run it from the repo root; it needs `yq`
on PATH and it writes only under its own temp directory.

```bash
S="$(mktemp -d "${TMPDIR:-/tmp}/i2m-record.XXXXXX")"
cp -R tests/fixtures/inbox-to-memory/old-only/. "$S/"
printf '\nSee [[atlas-working-session-P5spzLt4Bz]] for background.\n' \
  >>"$S/notes/2025-11-04-atlas-scoping-call-3iMu15QJ_x.md"
git -C "$S" init -q && git -C "$S" add -A
git -C "$S" -c user.email=t@t -c user.name=t commit -qm baseline
SINCE="$(git -C "$S" rev-parse HEAD)"
bash inbox-to-memory/scripts/migrate-scope.sh "$S" --apply >/dev/null
git -C "$S" add -A
git -C "$S" -c user.email=t@t -c user.name=t commit -qm migrated
bash inbox-to-memory/scripts/verify-migration.sh "$S" --since "$SINCE"; echo "exit=$?"

# C-3 needs the streams apart; re-run split when checking it.
bash inbox-to-memory/scripts/verify-migration.sh "$S" --since "$SINCE" \
  >"$S.out" 2>"$S.err"
rm -rf "$S" "$S.out" "$S.err"
```

It exits 0 and checks exactly one link, by id fallback — the count-1 case the
issue is about. Verified against the tree at `5f00eea` while this constitution
was being written.
...
None. The two strings a protected-passages manifest would normally guard —
`1 links checked` and `Paste this paragraph into the scope's patterns journal.` —
are exactly what this job exists to change. Pinning them again is how the defect
shipped in the first place; C-4 through C-6 and C-11 protect the assertions'
*strength* instead of any literal's text, which is the property that was
actually wanted.

## Non-goals

- The sweep's checks. Link resolution, the lint pass-through, rename and deletion handling, and every counter's computed value are out of scope (C-7).
- The summary block's grammar. `links checked: N (id fallback: M)` stays as it is, by the user's decision — it is machine output that two assertions read as such.
- The `lint failures`, `renames`, and `deletions` counters' wording. All three are pinned at zero in any record, so none of them can print the disagreeing form the issue is about. Changing them is not required, and doing so must not break their assertions.
- Writing to `patterns-journal/`. The record is stdout-only and stays that way; the suite has a check for exactly this and it keeps passing.
- Adding a `refute_output` helper to the suite. C-11 can be satisfied with the idioms already in the file; a new shared helper is a wider change than this job wants.
- Any other script, skill, or test file (C-9).
