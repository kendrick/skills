...
- **One confirmation, then it doesn't ask again, with two exceptions.** The shape, the isolation, the red-team mode, and the push grant are one question. Everything after yes—dispatching, committing, pushing, opening the pull request, pushing repair commits—happens unattended. `adversarial-review` can ask again, when the real diff trips a review the confirmation didn't forecast or forecast at a lower depth. The budget stop can ask too. Each step stamps its start and end in `timing.log`, and once review time passes twice the build time, the run stops before the next review cycle and asks whether to raise the ratio. Either way, the run waits until you answer. Use `--dry-run` to see the question without answering it.
...
- **`adversarial-review` fires only when the diff earns it.** It pipes the committed diff to `adversarial-review`'s own `match-triggers.py`. The script checks the changed lines against the money, authz, and schema rows of that skill's trigger table, reading the table at run time so the rows stay current. `--deep` fires the review outright. A docs-and-types diff gets the lightweight reproducer and nothing heavier. After a review repair it fires again only if the round being repaired held a P0, P1, or blocking finding. A repair of lower-severity findings gets the reproducer alone. Without `adversarial-review` installed there's no script to run and no table to read. The run checks for both at its opening gate, and where either is missing it stops before anything is built and names the skill to install.
