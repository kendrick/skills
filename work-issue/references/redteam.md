...
**After a repair, the trigger re-fires only where the triage round that repair answered held a row whose Severity is `P0`, `P1`, or `blocking`.** Step 8 item 1 always runs the reproducer over the repair's claims. It re-evaluates the trigger only for such a round, and a round of lower-severity rows ends at the reproducer. Where it does re-evaluate, the files are the repair's own: `RUN_DIR/redteam/trigger-repair-<k>.txt` and `RUN_DIR/redteam/ar-state-repair-<k>.txt`, with `k` the triage round, in the same formats as above. `trigger.txt` and `ar-state.txt` stay the build's record. Row 11 reads those two files, and row 17 reads the repair's. Each fix is new code, and new code draws an advisory a notch smaller than the last, so a review that re-fired after every repair almost never came back empty. On cambium #23 and #26 that took 7 repair cycles per lane and about 5 h of review against about 50 min of build, for 1 blocker among about 30 reproduced findings.
