...
| `review_over_budget` | `case "$(work-issue/scripts/run-state.py budget --timing <RUN_DIR>/timing.log)" in *'over: yes') echo true;; *'over: no') echo false;; *) echo null;; esac`. Never `null` on a run that predates the log, because `budget` answers a missing log with `over: no`. An unreadable or malformed log exits 3 with nothing on stdout, prints `null`, and stops the run naming this field rather than reading as under budget. No `--now`, unlike the in-run checks: a crashed step's open `start` closes at the last stamp the run recorded, so the hours it sat dead stay out of review |
...
Rows 10, 11, and 16, and row 17's unsettled-review leg, answer the budget stop (see SKILL.md's **Timing**) while `review_over_budget` is true, each reason keeping its `row <n>:` prefix. A higher ratio answers the stop by one of two routes. Appending `budget-raised <ratio>` to `timing.log` holds for this probe and every later check. Passing `--ratio <r>` to one `budget` command outranks every `budget-raised` line for that check alone, and the next probe reads the log's ratio again. Those four are the points that start another review cycle: a red-team round or its repair, an `adversarial-review` run, a triage repair, a repair's re-fired review. The rows after them publish, reply to, or report on work already paid for, and stopping there strands it: a repair left unpushed or a thread left unanswered costs the reviewer more than finishing costs the budget. Row 10's two-failed-rounds stop keeps its own reason, since it already stops.
