...
4. One pull-request comment headed `Deferred findings` carries the queue table copied byte-for-byte, never reformatted as a link or wrapped in backticks (see [references/triage.md](references/triage.md)), edited in place on later rounds rather than posted again. The resume probe compares every queue row's Source against that comment as literal text, so a round that added rows and stopped before this edit resumes here rather than at the poll.
