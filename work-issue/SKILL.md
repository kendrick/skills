...
7. Red-team mode for the confirmation text: `reproduce claims`, or `reproduce claims, then adversarial-review (<row names>)` when the Step 4 trigger grep hits over the plan's owned paths, or `reproduce claims, then adversarial-review (--deep)` when `--deep` is set, since that flag fires the trigger on its own. Step 4 re-runs that grep against the real diff, so this is the forecast rather than the verdict. Where the mode names `adversarial-review`, a yes to this confirmation also answers `adversarial-review`'s Step 2 question for this run.
8. The one confirmation, in one message:
   ...
   Once the user answers yes, write the red-team mode line this message carried to `RUN_DIR/redteam/mode.txt`, replacing any earlier copy. Step 4 reads that file to decide whether this yes answered `adversarial-review`'s Step 2 question, because a resumed run reaches Step 4 without this conversation.
