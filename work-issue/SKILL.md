...
**Done when:** every claim carries a reproducer verdict with its command and real output; every verdict says whether it was reproduced at the caller or at the seam, and a seam-only one carries the caller search that decided it; no claim is `NOT_REPRODUCED`, or the run stopped after two rounds; every `left` reason was checked against the code; `trigger.txt` is written; and where the trigger fired, `adversarial-review` completed with every blocker fixed or named in its `escalation.md`.
