...
**Timing.** Steps 1 through 8 each stamp `RUN_DIR/timing.log`: `echo "<n> start $(date -u +%FT%TZ)" >> RUN_DIR/timing.log` as step `<n>` begins, and the same line with `end` once its Done-when holds. Before any `start`, close a `start` of the same label that a crash left open, which shows as more `start` lines than `end` lines for that label: append `<n> end <latest>` first, `<latest>` being the newest stamp already in the log (`awk 'NF == 3 { print $3 }' RUN_DIR/timing.log | sort | tail -1`). The crashed interval then ends at the last thing the run recorded, and the hours it sat dead count toward nothing. Step 6 brackets each wait for a reviewer with `poll start` and `poll end`, inside its own `6` bracket and under the same close-first rule. The budget subtracts poll time from review, because the reviewer's latency is not this run's review. Read the log with:
