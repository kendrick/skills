...
- **Router + per-artifact profiles, not an 11.5 KB monolith.** Upstream ships one file; this skill splits static knowledge and dispatch into `SKILL.md` (held under 6.5 KB, roughly half upstream) and moves artifact-specific rules into `references/`, so a caller only loads the profile it needs.
