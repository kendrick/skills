...
| The verification-command harvest is a ladder, not a single manifest lookup | local | The command is the most load-bearing harvest row—gate 6 refuses an agent-targeted issue without a runnable one—and a repo whose test surface is shell scripts has no manifest to read. Leaving that case to inference broke Step 1's mechanical promise on every such repo, this one included. The rungs stay judgment-free: manifest scripts, then runnable scripts under `tests/` or `scripts/`, then what CI runs, then an explicit `absent`. |
...
| Parent and Blocked by ride in every template; blocking stays text, never GitHub-native | local, [C] | A prerequisite smuggled into a parenthetical sends an agent to run a file another ticket has yet to create, and a spec link left out throws away the cheapest context transfer there is. Text references keep the skill creation-only—native blocking relationships mean another API surface and edits beyond the one issue, for nothing a stranger can read that a `#N` reference doesn't already say. Blocked by states "None" outright on agent-targeted issues, because an absent list reads as unexamined rather than empty—the same absent-is-not-zero argument `inbox-to-memory` makes about its counts. Gate 6 backs the slot: an artifact named by the verification command or file pointers and missing from the repo has to appear on the Blocked by line. |
| The solo-repo cap scopes to the repo-shape trigger alone | local | Three of the five Depth 2 triggers are properties of the ask, one is a property of the workflow, and only one is a property of the tree. The unscoped cap let a solo repo talk an agent-destined, multi-component ask down to Depth 1, suppressing the code probe on exactly the issues that need file pointers, and it contradicted the floor-not-ceiling sentence directly above it. Scoping the cap keeps both rules rather than picking a winner. |
| Gate 6 requires verify-alone, with a declared blocker as the only exit | local, [P] | An issue scoped to one layer of a change passes all six gates and still cannot demonstrate itself: the agent finishes, cannot verify, and reports done regardless, which is the silent-wrong failure. Vertical slicing is `to-tickets`' central rule and practitioner consensus rather than a measured finding, so the evidence-map row carries [P]. The Depth 2 probe asks the prefactor question because it is already in the code when the question matters; sequencing prefactors into issues of their own stays with `to-tickets`. |
