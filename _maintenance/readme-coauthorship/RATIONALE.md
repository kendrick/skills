...
| The dispatch is pinned to polish time | eval | The first full pipeline run happened in a session that had already invoked `technical-writing` for a different artifact, and the run audited off the loaded router instead of dispatching fresh at Step 5. The audit still worked, which is what makes the failure quiet: the transcript reads compliant while the profile is whatever the session remembers. Cold sessions can't take the shortcut, so the wording targets the warm ones. |
