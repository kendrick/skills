...
The first sentence says where the reply landed. Four verdicts cover a thread the change answered—confirmed and fixed, confirmed and deferred, partly right, refused—so lead with one of them by name, then say what changed. A caller that fixes the opening outranks that: `work-issue` hands an in-scope fix `Fixed in <sha>.` and a queued row `deferred: <Outside because>`, and both stand as written rather than being bent back to a verdict name. The queued row is also the case those four do not reach, because `deferred` states a scope disposition and not a correctness verdict: queueing turns on scope alone, and an out-of-scope finding may be right, wrong, or never checked. Never read `deferred` as confirmation; where the finding's correctness is known too, the reply says that in its own clause.
