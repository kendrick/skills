...
Before writing `Overview.md`, stamp its `scaffold_digest`: strip the frontmatter, normalize CRLF to LF, SHA-256 the remaining bytes, store as `sha256:<hex>`. The hash covers everything after the closing `---` line—the delimiter and its newline excluded, the trailing newline included. Frontmatter stays out because Obsidian rewrites it unprompted—adding tags, touching timestamps—and none of that is a person writing content. A later check recomputes the hash, and an Overview whose digest still matches is one no person has written into.

Filing into an ID that already holds an `Overview.md` appends one line to its `## Log` per invocation—one line even when a confirmed batch lands many files at once. One file gets the by-name line: `- **YYYY-MM-DD** — filed <filename>`. Two or more get a count and a provenance instead: `- **2026-09-02** — filed 34 notes from Notion export 2019–2021`. The batch line names no files, because the Log records what this skill did, not what the ID contains, and one confirmed batch is one act. Append only—never rewrite existing lines, never touch other sections. Frontmatter is the one exception: appending to the Log changes the body, so re-stamp `scaffold_digest` from the new bytes in the same motion. Skip the re-stamp and the next check reads this skill's own append as a human edit. An inventory would go stale the first time a human adds a file by hand—the register anti-pattern one level down.
...
**Done when:** the register line exists, matches `[register].entry`, sits under the correct headings, and its purpose clause would settle the next arrival without naming any current file; every substrate folder listed on that line exists with the identical name; the thing is inside one of them; a newly minted ID's vault folder holds an `Overview.md`, built from the template, whose `scaffold_digest` matches its body—as does every Overview this run appended a Log line to; and, for a category `[moc].categories` names, its map table includes the new ID.
