...
1. **Signals a changed line can match.** "Touches the payment flow" is not a signal; `stripe`, `charge(`, `refund` are. Write each one in backticks in the signals cell: `match-triggers.py` takes the backticked strings as the row's signals and ignores the prose around them. Keep the `| # | Row |` header and the separator row as they are, since the script finds the table by them and exits 1 when it cannot.
