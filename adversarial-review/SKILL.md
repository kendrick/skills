...
That run is **waived**: the yes covered the fan-out's cost, and the user never saw the territories or the list the finders read. A settled decision missing from the list can come back as a reproduced blocker, so a waived run escalates its blockers at Step 6 instead of fixing them.
