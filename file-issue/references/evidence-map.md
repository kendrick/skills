...
| Falsifier for a derived rule | Q3's implication names testability and the absence of referential ambiguity as the attribute the evidence supports, and a derivation rule written in prose can miss it invisibly — naming which field was absent where it meant which field the value traces to, and reading correctly either way. The extension from acceptance criteria to the rule behind a derived value is not in the research: the Rubric Derivation Table carries no candidate row for a derivation, classification, or scoring rule, and no Gaps Register entry covers one. Found by one real run (#110); nothing measures whether naming the falsifier changes outcomes. | [C] | — |
...
- **The size check is unmeasured.** The ≥4-component threshold is [C]. Context fit is [P] by way of `to-tickets`' slicing practice—a property of the work the drafting agent cannot move by rephrasing—and the acceptance-criteria count was demoted to a smell for exactly that reason: the count measured the write-up. No study ties any of the three to issue outcomes.
