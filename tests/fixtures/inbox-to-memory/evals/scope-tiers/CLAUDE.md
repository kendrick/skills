# Claude Context — Calderwood Mutual
...
>
> **Memory mode:** lightweight (Decision, Context, Rule).

---

## The Client in One Paragraph

Calderwood Mutual is a mutual insurer writing home and motor cover across the UK and Ireland, about 1.4 million policies in force and roughly 900 staff across two sites. Slalom has been on site since October 2025. Two engagements are live: a delivery project replacing the claims intake path, and a pre-sale pursuit for an underwriting decision-support pilot.

## The User's Role

Engagement lead across both pieces of work. Reports into the Slalom account lead, not into Calderwood. Owns the delivery plan for claims-intake and the proposal for underwriting-assist; does not own anything inside Calderwood's own governance.

## Key Context About Calderwood Mutual

Mutual, so no shareholder pressure and a long tolerance for slow change — the policy administration platform, Kestrel, went in around 1994 and still holds the policy master. The organisation is genuinely two organisations: an operations side under the COO that runs claims and underwriting, and a technology side under the CIO that runs the platforms those functions sit on. Budget follows the function rather than the platform, which is why an operations director can fund a system and IT can only advise on it. Regulated business, so anything touching member data goes through an information-governance gate that sits outside both sides.
...
├── _memory/                      # client-wide memory (decisions/, context/, rules/)
...
- **Client-wide memory** at `_memory/` (subfolders: decisions/, context/, rules/). Records here describe facts that hold across multiple of this client's pursuits and projects — stakeholder relationships, org chart, comms cadence, tech-environment facts.
...
- **Watch for journal candidates during all work.** When you observe something that might generalize beyond Calderwood Mutual (a methodology pattern, an anti-pattern, a coaching insight), flag it inline with `[journal candidate: ...]` before continuing. Catch them as they occur; don't try to re-detect at session end.
...
- Memory records use the lightweight schema (Decision, Context, Rule; statuses `proposed | accepted | superseded | deprecated | rejected`).
...
- **Dated entries use ISO format.** `2026-05-11`, not `May 11, 2026`.
...
<!-- Calderwood Mutual-specific overrides. Fill in during scaffold or as the client engagement evolves. -->

### Active Engagements

- `projects/claims-intake/` — delivery. Replacing the counter and phone intake path with Fieldnote. Started 2026-01-12, go-live in the 2026 SOW as milestone 6.
- `pursuits/underwriting-assist/` — pre-sale. Lantern, a decision-support pilot for referred and declined underwriting cases. Proposal due 2026-06-30.
...
- Nadia Okonjo — Head of Information Governance. Chairs the data-sharing review board. Sits under the Company Secretary rather than under IT or Operations. Email only; declines standing invites on principle.
- Hal Brenner — Kestrel platform lead. The only person who can speak to what the policy master actually holds. Slow to reply, always replies.
...
```
Nadia Okonjo <- [Nadya, N. Okonjo, Nadia Okonjwo]
Kestrel <- [Kestral, Kestrel PAS, the PAS]
Dermot Sayer <- [Dermot Sayers, D. Sayer]
```
...
`calderwood`, `governance`, `kestrel`, `claims`, `underwriting`, `member-data`
