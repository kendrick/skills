...
Scaffolding doesn't count as payload, and a file has to pass two tests to qualify: its basename appears in `[rules].scaffold_names`, and it carries a `scaffold_digest` that still matches its body. `jd-file` drops an `Overview.md` into every ID it mints, and `inbox-to-memory` writes a `CLAUDE.md` and a `README.md` beside it, so without this test every reservation would look occupied by its own furniture.

Both halves do work. The name list is a cheap gate, so the check never opens a file it has no reason to read. It is also what makes a name like `README.md` safe to list at all—furniture inside a scaffold, somebody's own note everywhere else. The digest decides which one is in front of you. Write real content into a scaffold note and the digest stops matching, so the file counts as payload and the ID drops off the list.

Every way of failing to prove furniture lands on payload: a file that can't be read, one carrying no frontmatter, one that was never stamped, one whose digest has gone stale. The asymmetry is deliberate. Calling payload furniture would make an occupied ID report empty, and an ID on the empty list is one somebody might propose retiring.
